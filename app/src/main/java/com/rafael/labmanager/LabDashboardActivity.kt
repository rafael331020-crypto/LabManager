...
class LabDashboardActivity : AppCompatActivity() {
    ...
    private fun build(){
        ...
        stat(root,"ATENDIMENTO","Pacientes • requisições • convênios",blue){openModules()}
        stat(root,"AMOSTRAS","Coleta • triagem • rastreabilidade",orange){openModules()}
        stat(root,"ÁREA TÉCNICA","Resultados • conferência • liberação",green){openModules()}
        stat(root,"GESTÃO","Estoque • qualidade • financeiro",navy){openModules()}
        ...
        val open=MaterialButton(this).apply{text="ABRIR CENTRAL DO LIS";isAllCaps=false;textSize=15f;setTextColor(Color.WHITE);backgroundTintList=android.content.res.ColorStateList.valueOf(blue);cornerRadius=dp(12);minimumHeight=dp(54);setOnClickListener{openModules()}}
    ...
    private fun openModules(){startActivity(Intent(this,ProfessionalModulesActivity::class.java))}
}
